...
const components = {
  ...
  td: (props: any) => (
    <td
      className="border-border border-t px-4 py-2 text-muted-foreground"
      {...props}
    />
  ),
  p: (props: any) => (
  ...
  Note: (props: any) => (
    <div
      className={cn(
        "mt-4 rounded-md border-border border-l-4 bg-muted px-4 py-1 text-[0.95rem] text-foreground leading-[1.4rem]",
        {
  ...
  Prerequisites: (props: any) => (
    <div className="mt-4 rounded-md border border-border bg-card px-6 py-1 text-[0.95rem] leading-[1.4rem] shadow-md">
      <div className="-mb-6 flex items-center space-x-2 text-muted-foreground">
        <ListChecks size={20} />
        <p className="font-medium text-sm uppercase">Forutsetninger</p>
      </div>
      {props.children}
    </div>
  ),
      ...
      {[...allBlogPosts, ...allChangelogPosts]
        ...
        .map(post => (
          <li key={post.slug}>
            ...
            >
              <div>
                <p className="font-medium text-muted-foreground text-xs group-hover:text-foreground">
                  {formatDate(post.publishedAt)}
                </p>
                <h3 className="my-px font-medium text-base text-foreground group-hover:text-foreground">
                  {post.title}
                </h3>
                <p className="line-clamp-1 text-muted-foreground text-sm group-hover:text-foreground">
                  {post.summary}
                </p>
              </div>
