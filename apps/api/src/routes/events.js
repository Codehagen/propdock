...
import { sendDiscordNotification, sendSlackNotification, } from "../notifications/discord/sendDiscordNotification";
...
events.post("/", async (c) => {
    ...
    try {
        ...
        if (!project) {
            return c.json({
                ok: false,
                message: "No projects found for this user. Ensure the user has projects created.",
            }, 404);
        }
        ...
        const notificationSettings = await prisma(c.env).notificationSetting.findFirst({
        ...
        if (notificationSettings) {
            ...
            if (type === "DISCORD" && detailsParsed?.webhook) {
                await sendDiscordNotification(detailsParsed.webhook, `Event created: ${name}`);
            }
            else if (type === "SLACK" && detailsParsed?.webhook) {
                await sendSlackNotification(detailsParsed.webhook, `Event created: ${name}`);
            }
        }
        else {
            console.log("No notification settings found for the user.");
        }
        return c.json({ ok: true, message: "Event logged!", event: savedEvent }, 201); // Return 201 status code
    }
    catch (error) {
        return c.json({
            ok: false,
