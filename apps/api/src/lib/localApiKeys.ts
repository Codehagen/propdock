...
import { UserApiKey, WSApiKey, WorkspaceAccessToken } from "@prisma/client";


async function getUserApiKeyFull(env: Env, userId: string, serviceName: string): Promise<UserApiKey | null> {
    const db = prisma(env)
...
async function getUserApiKey(env: Env, userId: string, serviceName: string): Promise<string | null> {
...
async function getWSApiKeyFull(env: Env, workspaceId: string, serviceName: string): Promise<WSApiKey | null> {
    const db = prisma(env)
...
async function getWorkspaceApiKey(env: Env, workspaceId: string, serviceName: string): Promise<string | null> {
...
async function storeWorkspaceAccessToken(db: PrismaClient, workspaceId: string, serviceName: string, token: string, expiry: number): Promise<void> {
    const now = new Date();
...
async function getWorkspaceAccessToken(db: PrismaClient, workspaceId: string, serviceName: string): Promise<WorkspaceAccessToken | null> {
