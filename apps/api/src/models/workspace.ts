...
async function upsertAPIKey(db: PrismaClient, workspaceId: string, key: string, serviceName: string): Promise<void> {
...
async function upsertAccessToken(db: PrismaClient, workspaceId: string, token: string, expiry: any, serviceName: string): Promise<void> {
    const validTo = new Date(Date.now() + (expiry - 10) * 1000);
